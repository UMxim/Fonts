...
class FontGeneratorMenu:
    ...
    def save_to_header_file(self, glyphs, font_name, height):
        ...
        filename = f"font_{safe_font_name}_{height}.fnt"
        ...
        */
        ...
        with open(filename, 'w', encoding='utf-8') as f:
            ...
            all_bits = []
            bit_offsets = []
            current_bit_offset = 8  # Смещение от начала данных (c байта высоты)
            ...
            f.write(f"static const unsigned char font_data_{height}[] = {{\n")
            ...
            f.write("    unsigned short bit_offset;\n")
            f.write(f"}} glyphs_{height}[] = {{\n")

            for i, glyph in enumerate(sorted_glyphs):
                width = glyph['width']
                bit_offset = bit_offsets[i]  # Смещение в битах от начала данных
        ...
        print(f"Всего байт: {len(all_bytes) + 1}")  # +1 для байта высоты
