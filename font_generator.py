...
class FontGeneratorMenu:
    ...
    def save_to_header_file(self, glyphs, font_name, height):
        ...
        # Создаем безопасное имя шрифта для использования в идентификаторах
        ...
        # Имя файла без высоты
        filename = f"font_{safe_font_name}.h"
        
        # Имя для защиты от множественного включения
        include_guard = f"FONT_{safe_font_name.upper()}_H".replace(".", "_").replace("-", "_")
